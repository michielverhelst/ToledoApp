...
function setRSStemplate(dname, Body) {
	...
	for (var i = 0; i < entries.length; i++) {
		...
		//fill in the titles
		var output = Replace(rssBody.innerHTML, "(::Title::)", getNode('title', i)[0].childNodes[0].nodeValue);
		
		//fill in the link attribute
		var x = getNode('link', i)[1].attributes;
		var attribute = x.getNamedItem("href");
		output = Replace(output, "(::Link::)", attribute.value);
		
		//fill in the course
		var course = getNode('p', i)[2].childNodes[1].nodeValue;
		course = cleanCourse(course); //remove unwanted symbols
		output = Replace(output, "(::Course::)", course);
		
		//fill in the publish-date
		var publishedUTC = getNode('published', i)[0].childNodes[0].nodeValue;
